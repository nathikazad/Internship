...
Internal_RTC_clock::Internal_RTC_clock(int source)
{
	_source = source;
...
void Internal_RTC_clock::set_dateNtime(DateNTime dateNtime)
{
	_dateNtime=dateNtime;
	_hour = _dateNtime.get_hour();
	_minute = _dateNtime.get_minute();
	_second = _dateNtime.get_second();
	RTC_SetTime (RTC, _hour, _minute, _second);
	_day = _dateNtime.get_day();
	_month = _dateNtime.get_month();
	_year = _dateNtime.get_year()+2000;
...
DateNTime Internal_RTC_clock::get_dateNtime()
{
	_dateNtime.set_second(get_seconds());
	_dateNtime.set_minute(get_minutes());
	_dateNtime.set_hour(get_hours());
	_dateNtime.set_day(get_days());
	_dateNtime.set_month(get_months());
	_dateNtime.set_year(get_years()-2000);
	return _dateNtime;
}

uint32_t Internal_RTC_clock::current_time()
{
uint32_t dwTime;
...
int Internal_RTC_clock::get_hours()
{
...
int Internal_RTC_clock::get_minutes()
{
	_current_time = current_time();
...
int Internal_RTC_clock::get_seconds()
{
	_current_time = current_time();
 ...
 */
int Internal_RTC_clock::calculate_day_of_week(uint16_t _year, int _month, int _day)
{
	int _week;
...
uint32_t Internal_RTC_clock::current_date()
{
uint32_t dwTime;
...
uint16_t Internal_RTC_clock::get_years()
{
...
int Internal_RTC_clock::get_months()
{
	_current_date = current_date();
...
int Internal_RTC_clock::get_days()
{
	_current_date = current_date();
...
int Internal_RTC_clock::get_day_of_week()
{
	_current_date = current_date();
...
int Internal_RTC_clock::set_hours (int hour)
{
	_hour = hour;
...
int Internal_RTC_clock::set_minutes (int minute)
{
	_minute = minute;
	uint32_t _current_time = current_time();
	uint32_t _changed;
...
int Internal_RTC_clock::set_seconds (int second)
{
	_second = second;
	uint32_t _current_time = current_time();
	uint32_t _changed;
...
uint32_t Internal_RTC_clock::change_time (uint32_t now)
{
	_now = now;
...
int Internal_RTC_clock::set_days (int day)
{
	_day = day;
...
int Internal_RTC_clock::set_months (int month)
{
	_month = month;
	uint32_t _current_date = current_date();
	uint32_t _changed;
...
int Internal_RTC_clock::set_years (uint16_t year)
{
	_year = year;
	uint32_t _current_date = current_date();
	uint32_t _changed;
...
uint32_t Internal_RTC_clock::change_date (uint32_t now)
{
	_now = now;
...
void Internal_RTC_clock::attachalarm(void (*userFunction)(void))
{
...
void Internal_RTC_clock::set_alarmtime(int hour, int minute, int second)
{
	uint8_t _hour = hour;
...
void Internal_RTC_clock::set_alarmdate(int day, int month)
{
...
void Internal_RTC_clock::disable_alarm()
{
	RTC_DisableIt(RTC, RTC_IDR_ALRDIS);
}